...
<figure align="center">
  <img alt="KubeDB + KubeStash" src="/docs/v2026.6.18-rc.2/guides/qdrant/images/kubedb_plus_kubestash.svg">
...
<figure align="center">
  <img alt="Qdrant Backup Overview" src="/docs/v2026.6.18-rc.2/guides/qdrant/images/backup_overview.png">
...
13. Then, it creates the `Job` to backup the targeted `Qdrant` database.
...
15. Then, the `Job` dumps the targeted `Qdrant` database and uploads the output to the backend. KubeStash pipes the output of dump command to uploading process. Hence, backup `Job` does not require a large volume to hold the entire dump output.

16. After the backup process is completed, the backup `Job` updates the `status.components[dump]` field of the `Snapshot` resources with backup information of the target `Qdrant` database.
...
<figure align="center">
  <img alt="Database Restore Overview" src="/docs/v2026.6.18-rc.2/guides/qdrant/images/restore_overview.png">
...
- Backup a `Qdrant` database using KubeStash by following the guides from [here](/docs/v2026.6.18-rc.2/guides/qdrant/backup/logical/).
