...
- If you are not familiar with how KubeStash backup and restore Cassandra databases, please check the following guide [here](/docs/v2026.6.19/guides/cassandra/backup/kubestash/overview/).
...
Here, we have to use service `cas-sample` and secret `cas-sample-auth` to connect with the database. `KubeDB` creates an [AppBinding](/docs/v2026.6.19/guides/cassandra/concepts/appbinding) CR that holds the necessary information to connect with the database.
