...
Here, we have to use service `sample-singlestore` and secret `sample-singlestore-auth` to connect with the database. `KubeDB` creates an [AppBinding](/docs/v2026.6.19/guides/singlestore/concepts/appbinding) CR that holds the necessary information to connect with the database.
