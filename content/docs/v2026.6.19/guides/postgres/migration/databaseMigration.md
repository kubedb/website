...
- You should be familiar with the following `KubeDB` concepts:
    ...
    - [Migration](/docs/v2026.6.19/guides/postgres/concepts/migrator)
...
Here we connect to and migrate the `shop` database. Schema is extracted via `pg_dump` (`pgDump.schemaOnly: true`) and data is replicated using PostgreSQL logical replication with publication `pub` on the source and subscription `sub` on the target. For a full description of every field, see the [Migration CRD reference](/docs/v2026.6.19/guides/postgres/concepts/migrator).
