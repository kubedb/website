...
- You should be familiar with the following `KubeDB` concepts:
  ...
  - [Storage Autoscaling Overview](/docs/v2026.6.19/guides/milvus/autoscaler/storage/overview)
...
> To see the expansion fire, write enough data to push PVC usage past `usageThreshold` (30% here). When it does, the autoscaler creates a `VolumeExpansion` `MilvusOpsRequest` (with `expansionMode` as configured), which the Ops-manager applies — see the [volume expansion guide](/docs/v2026.6.19/guides/milvus/volume-expansion/guide) for the resulting flow and output.
...
- Learn about [compute autoscaling](/docs/v2026.6.19/guides/milvus/autoscaler/compute/guide).
