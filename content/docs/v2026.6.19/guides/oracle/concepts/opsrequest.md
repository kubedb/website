...
`OracleOpsRequest` is a Kubernetes `Custom Resource Definition` (CRD). It provides a declarative way to perform **day-2 operations** (such as restart, reconfigure, scaling, volume expansion and authentication rotation) on a KubeDB managed [Oracle](/docs/v2026.6.19/guides/oracle/concepts/oracle) database in a Kubernetes native way.
...
`spec.databaseRef` is a required field that points to the [Oracle](/docs/v2026.6.19/guides/oracle/concepts/oracle) object on which this operation will be applied. It has the following field:
...
- Learn about the [Oracle CRD](/docs/v2026.6.19/guides/oracle/concepts/oracle).
- Restart an Oracle database using an [OracleOpsRequest](/docs/v2026.6.19/guides/oracle/restart/restart).
