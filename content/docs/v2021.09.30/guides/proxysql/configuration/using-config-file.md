...
> **Note:** For this tutorial there must be a MySQL object with name `my-group` (Group Replication supported) running in the `demo` namespace in the cluster. You can deploy one by following section [create MySQL object with Group Replication](/docs/v2021.09.30/guides/proxysql/quickstart/load-balance-mysql-group-replication#Create-MySQL-Object).
