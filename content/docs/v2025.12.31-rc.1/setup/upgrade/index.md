...
kubectl apply -f https://github.com/kubeops/petset/raw/v0.0.10/crds/apps.k8s.appscode.com_petsets.yaml --server-side --force-conflicts
kubectl apply -f https://github.com/kubeops/petset/raw/v0.0.10/crds/apps.k8s.appscode.com_placementpolicies.yaml --server-side --force-conflicts
```

#### 2. Upgrade KubeDB Operator

Now, upgrade the KubeDB helm chart using the following command. You can find the latest installation guide [here](/docs/v2025.12.31-rc.1/setup/README). We recommend that you do **not** follow the legacy installation guide, as the new process is much more simpler.
...
Now, follow the latest installation guide to install the new version of the KubeDB operator. You can find the latest installation guide [here](/docs/v2025.12.31-rc.1/setup/README). We recommend that you do **not** follow the legacy installation guide, as the new process is much more simpler.
