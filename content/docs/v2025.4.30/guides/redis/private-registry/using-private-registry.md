...
- Read [concept of Redis Version Catalog](/docs/v2025.4.30/guides/redis/concepts/catalog) to learn detail concepts of `RedisVersion` object.
...
When installing KubeDB operator, set the flags `--docker-registry` and `--image-pull-secret` to appropriate value. Follow the steps to [install KubeDB operator](/docs/v2025.4.30/setup/README) properly in cluster so that to points to the DOCKER_REGISTRY you wish to pull images from.
