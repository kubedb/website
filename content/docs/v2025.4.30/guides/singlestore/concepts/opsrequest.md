...
`spec.databaseRef` is a required field that point to the [SingleStore](/docs/v2025.4.30/guides/singlestore/concepts/singlestore) object for which the administrative operations will be performed. This field consists of the following sub-field:
...
- `spec.updateVersion.targetVersion` refers to a [SingleStoreVersion](/docs/v2025.4.30/guides/singlestore/concepts/catalog) CR that contains the SingleStore version information where you want to update.

Have a look on the [`updateConstraints`](/docs/v2025.4.30/guides/singlestore/concepts/catalog#specupdateconstraints) of the singlestoreVersion spec to know which versions are supported for updating from the current version.
```yaml
