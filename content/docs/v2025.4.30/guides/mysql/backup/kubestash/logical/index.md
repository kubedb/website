...
Here, we have to use service `sample-mysql` and secret `sample-mysql-auth` to connect with the database. `KubeDB` creates an [AppBinding](/docs/v2025.4.30/guides/mysql/concepts/appbinding/) CR that holds the necessary information to connect with the database.
