...
`spec.version` is a required field specifying the name of the [MariaDBVersion](/docs/v2024.3.9-rc.0/guides/mariadb/concepts/mariadb-version) crd where the docker images are specified. Currently, when you install KubeDB, it creates the following `MariaDBVersion` resources,
