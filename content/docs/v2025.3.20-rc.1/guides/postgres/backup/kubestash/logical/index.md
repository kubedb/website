...
- If you are not familiar with how KubeStash backup and restore PostgreSQL databases, please check the following guide [here](/docs/v2025.3.20-rc.1/guides/postgres/backup/kubestash/overview/).
...
Here, we have to use service `sample-postgres` and secret `sample-postgres-auth` to connect with the database. `KubeDB` creates an [AppBinding](/docs/v2025.3.20-rc.1/guides/postgres/concepts/appbinding) CR that holds the necessary information to connect with the database.
