...
KubeDB comes with its own cli. It is called `kubedb` cli. `kubedb` can be used to manage any KubeDB object. `kubedb` cli also performs various validations to improve ux. To install KubeDB cli on your workstation, follow the steps [here](/docs/v2025.1.9/setup/README).
...
To learn about various options of `describe` command, please visit [here](/docs/v2025.1.9/reference/cli/kubectl-dba_describe).
...
Let's edit an existing running Memcached object to setup [Monitoring](/docs/v2025.1.9/guides/memcached/monitoring/using-builtin-prometheus). The following command will open Memcached `memcached-demo` in editor.
...
- Learn how to use custom configuration in Memcached with KubeDB [here](/docs/v2025.1.9/guides/memcached/custom-configuration/using-config-file)
