...
Now, install the KubeDB operator in your cluster following the steps [here](/docs/v2024.6.4/setup/install/_index).  and make sure install with helm command including `--set global.featureGates.Solr=true --set global.featureGates.ZooKeeper=true` to ensure Solr and ZooKeeper crd.
...
> We have designed this tutorial to demonstrate a production setup of KubeDB managed Solr. If you just want to try out KubeDB, you can bypass some safety features following the tips [here](/docs/v2024.6.4/guides/solr/quickstart/overview/#tips-for-testing).
...
- `AppBinding` - an [AppBinding](/docs/v2024.6.4/guides/solr/concepts/appbinding) which hold to connect information for the database. It is also named after the solr instance.
