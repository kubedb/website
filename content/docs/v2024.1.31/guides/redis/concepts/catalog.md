...
When you install KubeDB, a `RedisVersion` custom resource will be created automatically for every supported Redis versions. You have to specify the name of `RedisVersion` crd in `spec.version` field of [Redis](/docs/v2024.1.31/guides/redis/concepts/redis) crd. Then, KubeDB will use the docker images specified in the `RedisVersion` crd to create your expected database.
...
`metadata.name` is a required field that specifies the name of the `RedisVersion` crd. You have to specify this name in `spec.version` field of [Redis](/docs/v2024.1.31/guides/redis/concepts/redis) crd.
...
- Deploy your first Redis server with KubeDB by following the guide [here](/docs/v2024.1.31/guides/redis/quickstart/quickstart).
