...
Below is the YAML for MongoDB Replicaset. Here, [`spec.sslMode`](/docs/v2026.5.18-rc.0/guides/mongodb/concepts/mongodb#specsslMode) specifies `sslMode` for `replicaset` (which is `requireSSL`) and [`spec.clusterAuthMode`](/docs/v2026.5.18-rc.0/guides/mongodb/concepts/mongodb#specclusterAuthMode) provides `clusterAuthMode` for mongodb replicaset nodes (which is `x509`).
