...
`spec.version` is a required field that specifies the name of the [PostgresVersion](/docs/v2020.07.10-beta.0/concepts/catalog/postgres) crd where the docker images are specified. Currently, when you install KubeDB, it creates the following `PostgresVersion` resources,
...
To learn more about how to setup a HA PostgreSQL cluster in KubeDB, please visit [here](/docs/v2020.07.10-beta.0/guides/postgres/clustering/ha_cluster).
...
Follow [this link](/docs/v2020.07.10-beta.0/concepts/snapshot#google-cloud-storage-gcs) to learn how to create secret for S3 or GCS. To learn more about how to configure Postgres to archive WAL data continuously in AWS S3 bucket, please visit [here](/docs/v2020.07.10-beta.0/guides/postgres/snapshot/wal/continuous_archiving).
...
In the above example, Postgres will execute provided script once the database is running. For more details tutorial on how to initialize from script, please visit [here](/docs/v2020.07.10-beta.0/guides/postgres/initialization/script_source).
...
For more details tutorial on how to initialize from snapshot, please visit [here](/docs/v2020.07.10-beta.0/guides/postgres/initialization/snapshot_source).
...
For more details tutorial on how to initialize from wal archive, please visit [here](/docs/v2020.07.10-beta.0/guides/postgres/initialization/wal/wal_source).

### spec.backupSchedule

KubeDB supports taking periodic snapshots for Postgres database. This is an optional section in `.spec`. When `spec.backupSchedule` section is added, KubeDB operator immediately takes a backup to validate this information. After that, at each tick KubeDB operator creates a [Snapshot](/docs/v2020.07.10-beta.0/concepts/snapshot) object. This triggers operator to create a Job to take backup.
...
- [Monitor PostgreSQL with builtin Prometheus](/docs/v2020.07.10-beta.0/guides/postgres/monitoring/using-builtin-prometheus)
- [Monitor PostgreSQL with CoreOS Prometheus operator](/docs/v2020.07.10-beta.0/guides/postgres/monitoring/using-coreos-prometheus-operator)

### spec.configSource

`spec.configSource` is an optional field that allows users to provide custom configuration for PostgreSQL. This field accepts a [`VolumeSource`](https://github.com/kubernetes/api/blob/release-1.11/core/v1/types.go#L47). You can use any kubernetes supported volume source such as `configMap`, `secret`, `azureDisk` etc. To learn more about how to use a custom configuration file see [here](/docs/v2020.07.10-beta.0/guides/postgres/custom-config/using-custom-config).
...
If a service account name is given, and there's an existing service account by that name, the KubeDB operator will use that existing service account. Since this service account is not managed by KubeDB, users are responsible for providing necessary access permissions manually. Follow the guide [here](/docs/v2020.07.10-beta.0/guides/postgres/custom-rbac/using-custom-rbac) to grant necessary permissions in this scenario.
...
- Learn how to use KubeDB to run a PostgreSQL database [here](/docs/v2020.07.10-beta.0/guides/postgres/README).
