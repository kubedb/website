...
When you install KubeDB, a `ProxySQLVersion` custom resource will be created automatically for supported ProxySQL versions. You have to specify the name of `ProxySQLVersion` object in `.spec.version` field of [ProxySQL](/docs/v2020.07.10-beta.0/concepts/database-proxy/proxysql) object. Then, KubeDB will use the docker images specified in the `ProxySQLVersion` object to create your ProxySQL instance.
...
`.metadata.name` is a required field that specifies the name of the `ProxySQLVersion` object. You have to specify this name in `.spec.version` field of [ProxySQL](/docs/v2020.07.10-beta.0/concepts/database-proxy/proxysql) object.
...
- Learn about ProxySQL CRD [here](/docs/v2020.07.10-beta.0/concepts/database-proxy/proxysql).
- Deploy your first ProxySQL to load balance MySQL Group Replication with KubeDB by following the guide [here](/docs/v2020.07.10-beta.0/guides/proxysql/quickstart/load-balance-mysql-group-replication).
