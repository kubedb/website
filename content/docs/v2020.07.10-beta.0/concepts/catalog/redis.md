...
When you install KubeDB, a `RedisVersion` custom resource will be created automatically for every supported Redis versions. You have to specify the name of `RedisVersion` crd in `spec.version` field of [Redis](/docs/v2020.07.10-beta.0/concepts/databases/redis) crd. Then, KubeDB will use the docker images specified in the `RedisVersion` crd to create your expected database.
...
`metadata.name` is a required field that specifies the name of the `RedisVersion` crd. You have to specify this name in `spec.version` field of [Redis](/docs/v2020.07.10-beta.0/concepts/databases/redis) crd.
...
- Learn about Redis crd [here](/docs/v2020.07.10-beta.0/concepts/databases/redis).
- Deploy your first Redis server with KubeDB by following the guide [here](/docs/v2020.07.10-beta.0/guides/redis/quickstart/quickstart).
