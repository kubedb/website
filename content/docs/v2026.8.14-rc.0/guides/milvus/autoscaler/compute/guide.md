...
- You should be familiar with the following `KubeDB` concepts:
  ...
  - [Compute Autoscaling Overview](/docs/v2026.8.14-rc.0/guides/milvus/autoscaler/compute/overview)
...
The Ops-manager then applies the vertical scaling exactly as in the [vertical scaling guide](/docs/v2026.8.14-rc.0/guides/milvus/scaling/vertical-scaling/guide), right-sizing the pod to the recommended resources.
...
The behavior is identical to standalone, except a VPA object and resource recommendation are produced **per role**. When any role's recommendation differs from its current resources by more than `resourceDiffPercentage`, the autoscaler creates a `VerticalScaling` `MilvusOpsRequest` scoped to that role (see the [vertical scaling guide](/docs/v2026.8.14-rc.0/guides/milvus/scaling/vertical-scaling/guide) for what that ops request looks like).
...
- Learn about [storage autoscaling](/docs/v2026.8.14-rc.0/guides/milvus/autoscaler/storage/guide).
