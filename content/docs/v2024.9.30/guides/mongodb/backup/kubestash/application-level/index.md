...
Here, we have to use service `sample-mongodb` and secret `sample-mongodb-auth` to connect with the database. `KubeDB` creates an [AppBinding](/docs/v2024.9.30/guides/mongodb/concepts/appbinding) CR that holds the necessary information to connect with the database.
