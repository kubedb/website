...
- Read [concept of ProxySQLVersion Catalog](/docs/v2020.10.27-rc.0/guides/proxysql/concepts/catalog) to learn detail concepts of `ProxySQLVersion` object.
...
When installing KubeDB operator, set the flags `--docker-registry` and `--image-pull-secret` to the appropriate value. Follow the steps to [install KubeDB operator](/docs/v2020.10.27-rc.0/setup/README) properly in the cluster so that to points to the DOCKER_REGISTRY you wish to pull images from.
