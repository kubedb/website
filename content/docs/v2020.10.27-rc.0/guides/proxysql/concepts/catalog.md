...
`.metadata.name` is a required field that specifies the name of the `ProxySQLVersion` object. You have to specify this name in `.spec.version` field of [ProxySQL](/docs/v2020.10.27-rc.0/guides/proxysql/concepts/proxysql) object.
...
- Learn about ProxySQL CRD [here](/docs/v2020.10.27-rc.0/guides/proxysql/concepts/proxysql).
- Deploy your first ProxySQL to load balance MySQL Group Replication with KubeDB by following the guide [here](/docs/v2020.10.27-rc.0/guides/proxysql/quickstart/load-balance-mysql-group-replication).
