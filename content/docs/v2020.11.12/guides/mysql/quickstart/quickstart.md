...
Learn details of all `TerminationPolicy` [here](/docs/v2020.11.12/guides/mysql/concepts/mysql#specterminationpolicy).
...
When the [TerminationPolicy](/docs/v2020.11.12/guides/mysql/concepts/mysql#specterminationpolicy) is set to `Delete` and the MySQL object is deleted, the KubeDB operator will delete the StatefulSet and its pods along with PVCs but leaves the `secret` and database backup data(`snapshots`) intact.
...
2. **Use `terminationPolicy: WipeOut`**. It is nice to be able to delete everything created by KubeDB for a particular MySQL crd when you delete the crd. For more details about termination policy, please visit [here](/docs/v2020.11.12/guides/mysql/concepts/mysql#specterminationpolicy).
