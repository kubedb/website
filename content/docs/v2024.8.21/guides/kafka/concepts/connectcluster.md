...
`spec.version` is a required field specifying the name of the [KafkaVersion](/docs/v2024.8.21/guides/kafka/concepts/kafkaversion) CR where the docker images are specified. Currently, when you install KubeDB, it creates the following `KafkaVersion` resources,
...
- Monitor your ConnectCluster with KubeDB using [`out-of-the-box` Prometheus operator](/docs/v2024.8.21/guides/kafka/monitoring/using-prometheus-operator).
