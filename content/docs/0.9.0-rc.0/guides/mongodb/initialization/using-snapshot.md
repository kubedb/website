...
- This tutorial assumes that you have created a namespace `demo` and a snapshot `snapshot-infant`. Follow the steps [here](/docs/0.9.0-rc.0/guides/mongodb/snapshot/backup-and-restore) to create a database and take [instant snapshot](/docs/0.9.0-rc.0/guides/mongodb/snapshot/backup-and-restore#instant-backups), if you have not done so already. If you have changed the name of either namespace or snapshot object, please modify the YAMLs used in this tutorial accordingly.
