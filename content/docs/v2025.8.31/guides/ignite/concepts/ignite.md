...
`spec.version` is a required field specifying the name of the [IgniteVersion](/docs/v2025.8.31/guides/ignite/concepts/ignite-version) crd where the docker images are specified. Currently, when you install KubeDB, it creates the following `IgniteVersion` crds,
...
- [Monitor Ignite with builtin Prometheus](/docs/v2025.8.31/guides/ignite/monitoring/using-builtin-prometheus)
- [Monitor Ignite with Prometheus operator](/docs/v2025.8.31/guides/ignite/monitoring/using-prometheus-operator)

### spec.configSecret

`spec.configSecret` is an optional field that allows users to provide custom configuration for Ignite. This field accepts a [`VolumeSource`](https://github.com/kubernetes/api/blob/release-1.11/core/v1/types.go#L47). So you can use any Kubernetes supported volume source such as `configMap`, `secret`, `azureDisk` etc. To learn more about how to use a custom configuration file see [here](/docs/v2025.8.31/guides/ignite/custom-configuration/using-config-file).
...
`KubeDB` provides the flexibility of deploying Ignite server from a private Docker registry. To learn how to deploym Ignite from a private registry, please visit [here](/docs/v2025.8.31/guides/ignite/private-registry/using-private-registry).
...
If a service account name is given, and there's an existing service account by that name, the KubeDB operator will use that existing service account. Since this service account is not managed by KubeDB, users are responsible for providing necessary access permissions manually. Follow the guide [here](/docs/v2025.8.31/guides/ignite/custom-rbac/using-custom-rbac) to grant necessary permissions in this scenario.
...
- Learn how to use KubeDB to run a Ignite server [here](/docs/v2025.8.31/guides/ignite/README).
