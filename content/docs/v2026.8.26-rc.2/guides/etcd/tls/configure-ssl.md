...
- Now, install KubeDB cli on your workstation and KubeDB operator in your cluster following the steps [here](/docs/v2026.8.26-rc.2/setup/README). Etcd support is behind an alpha feature gate, so make sure the operator was installed with `--set featureGates.Etcd=true`.
...
Read about the fields in detail in the [Etcd Concept Guide](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcd#spectls), and about what each certificate alias secures in the [TLS overview](/docs/v2026.8.26-rc.2/guides/etcd/tls/overview).
...
> If you have enabled etcd RBAC on this cluster (which happens the first time you run a [`RotateAuth` EtcdOpsRequest](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcdopsrequest)), add `--user=root:<password>` to the commands above. The password lives in the `<db-name>-auth` secret.
...
- Add, rotate or remove TLS on a **running** cluster with a [ReconfigureTLS EtcdOpsRequest](/docs/v2026.8.26-rc.2/guides/etcd/reconfigure-tls/reconfigure-tls).
- Monitor your etcd cluster with [builtin Prometheus](/docs/v2026.8.26-rc.2/guides/etcd/monitoring/using-builtin-prometheus) or the [Prometheus operator](/docs/v2026.8.26-rc.2/guides/etcd/monitoring/using-prometheus-operator).
- Tune etcd at creation time with [custom configuration](/docs/v2026.8.26-rc.2/guides/etcd/custom-configuration/using-config).
