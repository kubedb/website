...
9. Finally it rolls the member pods so that they come up with the new flags. This reuses exactly
   the same leader-aware sequence as the [Restart](/docs/v2026.8.26-rc.2/guides/etcd/restart/restart) ops
...
> [`spec.configuration`](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcd#specconfiguration) in the Etcd concept
> doc. This guide is only about changing them on an already running cluster with an
> `EtcdOpsRequest`.

In the [next](/docs/v2026.8.26-rc.2/guides/etcd/reconfigure/reconfigure) doc, we are going to show a step by step
