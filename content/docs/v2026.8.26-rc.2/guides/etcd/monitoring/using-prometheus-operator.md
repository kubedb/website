...
> **Read this first:** unlike most KubeDB databases, etcd has **no exporter sidecar**. etcd serves Prometheus metrics natively on its own metrics listener at port `2381`. KubeDB only creates the stats `Service` and the `ServiceMonitor` that point at it — nothing is added to the pod. See the [monitoring overview](/docs/v2026.8.26-rc.2/guides/etcd/monitoring/overview).
...
- Monitor your etcd cluster with KubeDB using [builtin Prometheus](/docs/v2026.8.26-rc.2/guides/etcd/monitoring/using-builtin-prometheus).
