...
No CRD is involved. See [Monitor Etcd using builtin Prometheus](/docs/v2026.8.26-rc.2/guides/etcd/monitoring/using-builtin-prometheus).

### `prometheus.io/operator`

KubeDB additionally creates a `ServiceMonitor` named `<etcd-crd-name>-stats`, **in the same namespace as the `Etcd` object**, selecting the stats Service. See [Monitor Etcd using Prometheus operator](/docs/v2026.8.26-rc.2/guides/etcd/monitoring/using-prometheus-operator).
...
The backend-size-versus-quota pair is the one most worth wiring an alert to, because hitting the quota puts the cluster into a read-only alarm state that needs manual intervention. See [custom configuration](/docs/v2026.8.26-rc.2/guides/etcd/custom-configuration/using-config) for how to set the quota and the auto-compaction policy, and the `Compact`/`Defragment` [EtcdOpsRequest](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcdopsrequest) types for reclaiming space.
...
- Monitor your etcd cluster with KubeDB using the [Prometheus operator](/docs/v2026.8.26-rc.2/guides/etcd/monitoring/using-prometheus-operator).
