...
`spec.version` is a required field specifying the name of the [EtcdVersion](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcdversion) crd where the docker images are specified. Currently, when you install KubeDB, it creates the following `EtcdVersion` crds,
...
[EtcdOpsRequest](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcdopsrequest)) is reconciled against etcd's actual member list:
...
`spec.authSecret` also carries `rotateAfter`, which the Recommendation Engine uses to propose a `RotateAuth` [EtcdOpsRequest](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcdopsrequest) once the credential reaches the given age.

### spec.init

`spec.init` is an optional section that can be used to initialize a newly created Etcd cluster. For etcd, the interesting sub-section is `spec.init.archiver`, which restores the cluster from a KubeStash snapshot taken by an [EtcdArchiver](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcdarchiver).
...
> To restore into an `Etcd` object that already exists, use an `EtcdOpsRequest` of type `Restore`. It obeys the same rule rather than breaking it: the cluster is taken apart down to a single empty volume first, the snapshot is replayed into that, and the remaining members rejoin as new members. See [In-place Restore](/docs/v2026.8.26-rc.2/guides/etcd/restore/overview).
...
Enabling TLS switches the scheme of every advertised client and peer URL from `http` to `https`, so it is applied with a rolling restart. Use the `ReconfigureTLS` [EtcdOpsRequest](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcdopsrequest) to add, rotate or remove TLS on a running cluster.
...
`spec.archiver` opts the cluster into a scheduled snapshot backup driven by an [EtcdArchiver](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcdarchiver) policy object.
...
`status.authSecret` records the age of the credential currently referenced by `spec.authSecret`. Together with `spec.authSecret.rotateAfter`, this is what lets the Recommendation Engine propose a `RotateAuth` [EtcdOpsRequest](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcdopsrequest) when the credential gets old.

## Next Steps

- Learn how to use KubeDB to run an etcd cluster [here](/docs/v2026.8.26-rc.2/guides/etcd/README).
- Deploy your first etcd cluster with KubeDB by following the guide [here](/docs/v2026.8.26-rc.2/guides/etcd/quickstart/quickstart).
- Detail concepts of [EtcdVersion object](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcdversion).
- Detail concepts of [EtcdOpsRequest object](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcdopsrequest).
