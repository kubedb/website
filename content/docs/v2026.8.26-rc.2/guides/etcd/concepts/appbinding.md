...
When you deploy an [Etcd](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcd) object with KubeDB, an `AppBinding` object is created automatically for it. Backup and restore tooling — [KubeStash](https://kubestash.com/) and the etcd backup addon — uses it to find out how to reach the cluster and which credentials to use.
...
| `spec.version`                    | The etcd version, taken from the referenced [EtcdVersion](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcdversion).                          |

There is a single client Service rather than a primary/standby pair, because every etcd member serves the client API and forwards writes to the current Raft leader — see [Etcd CRD](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcd#specservicetemplates).

### When TLS is enabled

If [`spec.tls`](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcd#spectls) is configured on the `Etcd` object, KubeDB additionally fills in:
...
- Learn how to use KubeDB to manage various databases [here](/docs/v2026.8.26-rc.2/guides/README).
