...
`metadata.name` is a required field that specifies the name of the `EtcdVersion` crd. You have to specify this name in the `spec.version` field of the [Etcd](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcd) crd.
...
`spec.endOfLife` is an optional field that marks whether this etcd version has reached its end of life according to [endoflife.date](https://endoflife.date/). It is informational — it does not by itself block provisioning — but the Recommendation Engine takes it into account when proposing an `UpdateVersion` [EtcdOpsRequest](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcdopsrequest).
...
`spec.archiver` holds the KubeStash addon related specification used by the [EtcdArchiver](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcdarchiver) driven snapshot backup.
...
- Learn about the Etcd crd [here](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcd).
