...
`spec.databaseRef` is a required field that points to the [Etcd](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcd) object for which the administrative operations will be performed. This field consists of the following sub-field:

- **spec.databaseRef.name :** specifies the name of the [Etcd](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcd) object. The referenced object must live in the same namespace as the `EtcdOpsRequest`.
...
| Type                | What it does                                                                                                                                 |
|---------------------|------------------------------------------------------------------------------------------------------------------------------------------------|
| `UpdateVersion`     | Updates the cluster to a different [EtcdVersion](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcdversion), member by member.                                    |
...
- `spec.updateVersion.targetVersion` refers to an [EtcdVersion](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcdversion) CR that contains the etcd version information you want to update to.
...
- `spec.configuration.tuning` holds the KubeDB-managed etcd tuning knobs described in [Etcd CRD](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcd#specconfiguration). They end up on the etcd command line.
...
- `spec.tls.certificates` specifies the certificates. You can learn more about this field from [here](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcd#spectls). The valid aliases are `server`, `client`, `peer` and `metrics-exporter`.
...
If you would rather have this happen automatically, set `spec.configuration.tuning.autoCompactionMode` and `autoCompactionRetention` on the [Etcd](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcd#specconfiguration) object instead, and use this ops request only for one-off cleanups.
...
See the [Recover from Quorum Loss guide](/docs/v2026.8.26-rc.2/guides/etcd/recover-from-quorum-loss/recover-from-quorum-loss) for the full walkthrough.
...
This is the in-place counterpart of the **bootstrap-time** restore configured through `spec.init.archiver` on the [Etcd](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcd#specinit) object itself, which only works while a brand-new `Etcd` object is being created. If you are standing up a new cluster from a backup, use that one instead — see [Snapshot Backup & Restore](/docs/v2026.8.26-rc.2/guides/etcd/backup/kubestash/snapshot/#restore).
...
See the [In-place Restore guide](/docs/v2026.8.26-rc.2/guides/etcd/restore/restore) for the full walkthrough.
...
- Learn about the [Etcd](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcd) crd.
- Learn about the [EtcdVersion](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcdversion) crd.
