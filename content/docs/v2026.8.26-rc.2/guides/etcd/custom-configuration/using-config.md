...
This guide is about **day-1** configuration — the settings you bake into the `Etcd` object before the cluster exists. To change these on a cluster that is already running, use a `Reconfigure` [EtcdOpsRequest](/docs/v2026.8.26-rc.2/guides/etcd/reconfigure/reconfigure) instead; the fields are the same.

## Before You Begin

- At first, you need to have a Kubernetes cluster, and the kubectl command-line tool must be configured to communicate with your cluster. If you do not already have a cluster, you can create one by using [kind](https://kind.sigs.k8s.io/docs/user/quick-start/).

- Install KubeDB operator in your cluster following the steps [here](/docs/v2026.8.26-rc.2/setup/README). Etcd support is behind an alpha feature gate, so make sure the operator was installed with `--set featureGates.Etcd=true`.
...
- Monitor `etcd_mvcc_db_total_size_in_bytes` against `etcd_server_quota_backend_bytes` and alert well before the ratio reaches 1. See [monitoring](/docs/v2026.8.26-rc.2/guides/etcd/monitoring/overview).
...
> **Compaction alone does not shrink the file on disk.** It frees space *inside* the backend database for reuse, but the file stays the same size. To actually return the space to the filesystem you must defragment — use the `Defragment` [EtcdOpsRequest](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcdopsrequest), which walks the members one at a time (leader last) so the cluster never loses quorum while a member is blocked defragmenting.
...
> [RecoverFromQuorumLoss EtcdOpsRequest](/docs/v2026.8.26-rc.2/guides/etcd/recover-from-quorum-loss/overview),
...
Apply the change with a [Restart EtcdOpsRequest](/docs/v2026.8.26-rc.2/guides/etcd/restart/restart), which rolls
...
See the [Reconfigure guide](/docs/v2026.8.26-rc.2/guides/etcd/reconfigure/reconfigure) for the full walkthrough.
...
- Change the tuning knobs on a running cluster with a [Reconfigure EtcdOpsRequest](/docs/v2026.8.26-rc.2/guides/etcd/reconfigure/reconfigure).
- Watch the backend size against the quota with [monitoring](/docs/v2026.8.26-rc.2/guides/etcd/monitoring/overview).
- Run your etcd cluster with [TLS/SSL encryption](/docs/v2026.8.26-rc.2/guides/etcd/tls/configure-ssl).
- Detail concepts of [Etcd object](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcd).
