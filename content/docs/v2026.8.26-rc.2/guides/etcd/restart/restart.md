...
- Now, install KubeDB cli on your workstation and KubeDB operator in your cluster following the steps [here](/docs/v2026.8.26-rc.2/setup/README). Etcd support is behind an alpha feature gate, so make sure the KubeDB Provisioner and Ops-manager operators are installed with `featureGates.Etcd=true`.
...
- The meaning of `spec.timeout` & `spec.apply` fields is described [here](/docs/v2026.8.26-rc.2/guides/etcd/concepts/etcdopsrequest#spectimeout).
