...
`spec.license` is an optional field that names the Secret holding the license certificate a "Postgres Enterprise by AppsCode" build needs to start. This is unrelated to the KubeDB platform license you pass via `global.license` when installing the KubeDB operator ([see PostgresVersion](/docs/v2026.8.26-rc.2/guides/postgres/concepts/catalog)); that license activates KubeDB Enterprise features, while `spec.license` activates the database binary itself, and only applies when `spec.version` points at a [PostgresVersion](/docs/v2026.8.26-rc.2/guides/postgres/concepts/catalog) whose `spec.distribution` is `AppsCode` and `spec.license.required` is `true`. For any other distribution, `spec.license` must be left unset.
