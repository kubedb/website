---
title: Backup & Restore Neo4j Using KubeStash
menu:
  docs_v2026.8.26-rc.2:
    identifier: guides-neo4j-backup-overview-stashv2
    name: Overview
...
<figure align="center">
  <img alt="KubeDB + KubeStash" src="/docs/v2026.8.26-rc.2/guides/neo4j/backup/kubestash/overview/images/kubedb_plus_kubestash.svg">
<figcaption align="center">Fig: Backup KubeDB Databases Using KubeStash</figcaption>
</figure>

## How Backup Works

The following diagram shows how KubeStash takes backup of a `Neo4j` database. Open the image in a new tab to see the enlarged version.

<figure align="center">
  <img alt="Neo4j Backup Overview" src="/docs/v2026.8.26-rc.2/guides/neo4j/backup/kubestash/overview/images/backup_overview.svg">
  <figcaption align="center">Fig: Neo4j Backup Overview</figcaption>
</figure>
...
13. Then, it creates the `Job` to backup the targeted `Neo4j` database.
...
15. Then, the `Job` takes a backup of all the databases of the targeted `Neo4j` instance using the `neo4j-admin database backup` command and uploads the output to the backend. KubeStash pipes the output of the backup command to the uploading process. Hence, the backup `Job` does not require a large volume to hold the entire backup output.

16. After the backup process is completed, the backup `Job` updates the `status.components[dump]` field of the `Snapshot` resources with backup information of the target `Neo4j` database.

## How Restore Process Works

The following diagram shows how KubeStash restores backed up data into a `Neo4j` database. Open the image in a new tab to see the enlarged version.

<figure align="center">
  <img alt="Database Restore Overview" src="/docs/v2026.8.26-rc.2/guides/neo4j/backup/kubestash/overview/images/restore_overview.svg">
  <figcaption align="center">Fig: Neo4j Restore Process Overview</figcaption>
</figure>

The restore process consists of the following steps:

1. At first, a user creates a `Neo4j` database where the data will be restored or the user can use the same `Neo4j` database.
...
7. Then, the `Job` downloads the backed up data from the backend and restores it into the desired database using the seed server specified in the `RestoreSession`. KubeStash pipes the downloaded data to the respective restore tool to inject into the database. Hence, the restore `Job` does not require a large volume to download the entire backup data inside it.
...
- Backup a `Neo4j` database using KubeStash by the following guides from [here](/docs/v2026.8.26-rc.2/guides/neo4j/backup/kubestash/logical/).
