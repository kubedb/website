...
<p align="center">
  <img alt="lifecycle"  src="/docs/v2024.8.2-rc.2/images/kafka/connectcluster/connectcluster-crd-lifecycle.png">
</p>
...
Now, install the KubeDB operator in your cluster following the steps [here](/docs/v2024.8.2-rc.2/setup/install/_index).
...
> We have designed this tutorial to demonstrate a production setup of KubeDB managed Apache Kafka Connect Cluster. If you just want to try out KubeDB, you can bypass some safety features following the tips [here](/docs/v2024.8.2-rc.2/guides/kafka/quickstart/overview/connectcluster/#tips-for-testing).

## Find Available ConnectCluster Versions

When you install the KubeDB operator, it registers a CRD named [KafkaVersion](/docs/v2024.8.2-rc.2/guides/kafka/concepts/kafkaversion). ConnectCluster Version is using the KafkaVersion CR to define the specification of ConnectCluster. The installation process comes with a set of tested KafkaVersion objects. Let's check available KafkaVersions by,
...
Before create ConnectCluster, you have to deploy a `Kafka` cluster first. To deploy kafka cluster, follow the [Kafka Quickstart](/docs/v2024.8.2-rc.2/guides/kafka/quickstart/overview/kafka/) guide. Let's assume `kafka-quickstart` is already deployed using KubeDB.
...
- `AppBinding` - an [AppBinding](/docs/v2024.8.2-rc.2/guides/kafka/concepts/appbinding) which hold to connect information for the ConnectCluster worker nodes. It is also named after the ConnectCluster instance.
...
To create a connector, you can use the Kafka Connect REST API. But, KubeDB operator implements a `Connector` CRD to define the specification of a connector. Create a `Connector` CR to create a connector. Details of the [Connector](/docs/v2024.8.2-rc.2/guides/kafka/concepts/connector) CR.
...
Here, 
1. A MongoDB instance is already running. You can use your own MongoDB instance. To run mongodb instance, follow the [MongoDB Quickstart](/docs/v2024.8.2-rc.2/guides/mongodb/quickstart/quickstart) guide.
...
1 **Use `deletionPolicy: Delete`**. It is nice to be able to resume the cluster from the previous one. So, we preserve auth `Secrets`. If you don't want to resume the cluster, you can just use `spec.deletionPolicy: WipeOut`. It will clean up every resource that was created with the ConnectCluster CR. For more details, please visit [here](/docs/v2024.8.2-rc.2/guides/kafka/concepts/connectcluster#specdeletionpolicy).

## Next Steps

- [Quickstart Kafka](/docs/v2024.8.2-rc.2/guides/kafka/quickstart/overview/kafka/) with KubeDB Operator.
- [Quickstart ConnectCluster](/docs/v2024.8.2-rc.2/guides/kafka/quickstart/overview/connectcluster/) with KubeDB Operator.
- Use [kubedb cli](/docs/v2024.8.2-rc.2/guides/kafka/cli/cli) to manage databases like kubectl for Kubernetes.
- Detail concepts of [ConnectCluster object](/docs/v2024.8.2-rc.2/guides/kafka/concepts/connectcluster).
