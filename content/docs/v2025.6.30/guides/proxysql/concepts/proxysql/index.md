...
`.spec.version` is a required field specifying the name of the [ProxySQLVersion](/docs/v2025.6.30/guides/proxysql/concepts/proxysql-version/) CRD where the docker images are specified. Currently, when you install KubeDB, it creates the following `ProxySQLVersion` resources,
...
Checkout this [link](/docs/v2025.6.30/guides/proxysql/concepts/declarative-configuration/) for detailed overview on declarative configuration. 
