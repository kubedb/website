...
<p align="center">
  <img alt="lifecycle"  src="/docs/v2025.6.30/guides/mysql/quickstart/images/mysql-lifecycle.png">
</p>
...
When the [DeletionPolicy](/docs/v2025.6.30/guides/mysql/concepts/database/#specdeletionpolicy) is set to `Delete` and the MySQL object is deleted, the KubeDB operator will delete the PetSet and its pods along with PVCs but leaves the `secret` and database backup data(`snapshots`) intact.
...
2. **Use `deletionPolicy: WipeOut`**. It is nice to be able to delete everything created by KubeDB for a particular MySQL crd when you delete the crd. For more details about termination policy, please visit [here](/docs/v2025.6.30/guides/mysql/concepts/database/#specdeletionpolicy).
