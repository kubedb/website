...
`spec.version` is a required field specifying the name of the [MySQLVersion](/docs/0.9.0-rc.2/concepts/catalog/mysql) crd where the docker images are specified. Currently, when you install KubeDB, it creates the following `MySQLVersion` crd,
...
In the above example, KubeDB operator will launch a Job to execute all js script of `mysql-init-script` in alphabetical order once StatefulSet pods are running. For more details tutorial on how to initialize from script, please visit [here](/docs/0.9.0-rc.2/guides/mysql/initialization/script-source).
...
For more details tutorial on how to initialize from snapshot, please visit [here](/docs/0.9.0-rc.2/guides/mysql/initialization/snapshot-source).

### spec.backupSchedule

KubeDB supports taking periodic snapshots for MySQL database. This is an optional section in `.spec`. When `spec.backupSchedule` section is added, KubeDB operator immediately takes a backup to validate this information. After that, at each tick kubeDB operator creates a [Snapshot](/docs/0.9.0-rc.2/concepts/snapshot) object. This triggers operator to create a Job to take backup. If used, set the various sub-fields accordingly.
...
- [Monitor MySQL with builtin Prometheus](/docs/0.9.0-rc.2/guides/mysql/monitoring/using-builtin-prometheus)
- [Monitor MySQL with CoreOS Prometheus operator](/docs/0.9.0-rc.2/guides/mysql/monitoring/using-coreos-prometheus-operator)

### spec.configSource

`spec.configSource` is an optional field that allows users to provide custom configuration for MySQL. This field accepts a [`VolumeSource`](https://github.com/kubernetes/api/blob/release-1.11/core/v1/types.go#L47). So you can use any kubernetes supported volume source such as `configMap`, `secret`, `azureDisk` etc. To learn more about how to use a custom configuration file see [here](/docs/0.9.0-rc.2/guides/mysql/configuration/using-custom-config).
...
`KubeDB` provides the flexibility of deploying MySQL database from a private Docker registry. `spec.podTemplate.spec.imagePullSecrets` is an optional field that points to secrets to be used for pulling docker image if you are using a private docker registry. To learn how to deploy MySQL from a private registry, please visit [here](/docs/0.9.0-rc.2/guides/mysql/private-registry/using-private-registry).
...
- Learn how to use KubeDB to run a MySQL database [here](/docs/0.9.0-rc.2/guides/mysql/README).
