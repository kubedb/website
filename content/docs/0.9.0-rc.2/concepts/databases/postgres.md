...
`spec.version` is a required field that specifies the name of the [PostgresVersion](/docs/0.9.0-rc.2/concepts/catalog/postgres) crd where the docker images are specified. Currently, when you install KubeDB, it creates the following `PostgresVersion` crd,
...
`spec.replicas` specifies the total number of primary and standby nodes in Postgres database cluster configuration. One pod is selected as Primary and others are acted as standby replicas. To know more about how to setup a HA PostgreSQL cluster in KubeDB, please visit [here](/docs/0.9.0-rc.2/guides/postgres/clustering/ha_cluster).
...
To know more about how to configure Postgres to archive WAL data continuously in AWS S3 bucket, please visit [here](/docs/0.9.0-rc.2/guides/postgres/snapshot/continuous_archiving).
...
In the above example, Postgres will execute provided script once the database is running. For more details tutorial on how to initialize from script, please visit [here](/docs/0.9.0-rc.2/guides/postgres/initialization/script_source).
...
For more details tutorial on how to initialize from snapshot, please visit [here](/docs/0.9.0-rc.2/guides/postgres/initialization/snapshot_source).
...
For more details tutorial on how to initialize from wal archive, please visit [here](/docs/0.9.0-rc.2/guides/postgres/initialization/wal_source).

### spec.backupSchedule

KubeDB supports taking periodic snapshots for Postgres database. This is an optional section in `.spec`. When `spec.backupSchedule` section is added, KubeDB operator immediately takes a backup to validate this information. After that, at each tick kubeDB operator creates a [Snapshot](/docs/0.9.0-rc.2/concepts/snapshot) object. This triggers operator to create a Job to take backup.
...
You have to specify following fields to take periodic backup of your Postgres database:
 ...
 - `spec.backupSchedule.{storage}` is a required field that is used as the destination for storing snapshot data. KubeDB supports cloud storage providers like S3, GCS, Azure, and OpenStack Swift. It also supports any locally mounted Kubernetes volumes, like NFS, Ceph, etc. Only one backend can be used at a time. To learn how to configure this, please visit [here](/docs/0.9.0-rc.2/concepts/snapshot).
...
- Learn how to use KubeDB to run a PostgreSQL database [here](/docs/0.9.0-rc.2/guides/postgres/README).
