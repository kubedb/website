...
`ElasticsearchAutoscaler` is a Kubernetes `Custom Resource Definitions` (CRD). It provides a declarative configuration for autoscaling [Elasticsearch](/docs/v2022.12.24-rc.1/guides/elasticsearch/concepts/elasticsearch/) compute resources and storage of database components in a Kubernetes native way.
...
`spec.databaseRef` is a `required` field that point to the [Elasticsearch](/docs/v2022.12.24-rc.1/guides/elasticsearch/concepts/elasticsearch/) object for which the autoscaling will be performed. This field consists of the following sub-field:

- **spec.databaseRef.name :** specifies the name of the [Elasticsearch](/docs/v2022.12.24-rc.1/guides/elasticsearch/concepts/elasticsearch/) object.
