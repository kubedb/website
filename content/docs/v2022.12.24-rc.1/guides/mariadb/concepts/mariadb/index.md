...
`spec.version` is a required field specifying the name of the [MariaDBVersion](/docs/v2022.12.24-rc.1/guides/mariadb/concepts/mariadb-version) crd where the docker images are specified. Currently, when you install KubeDB, it creates the following `MariaDBVersion` resources,
