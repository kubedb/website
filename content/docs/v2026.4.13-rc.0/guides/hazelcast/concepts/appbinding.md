...
menu:
  docs_v2026.4.13-rc.0:
    identifier: hz-appbinding-hazelcast
    name: AppBinding
...
An `AppBinding` object created by `KubeDB` for Hazelcast database is shown below,
...
metadata:
  ...
  resourceVersion: "1179350"
  uid: ffaae659-be80-4934-abeb-c5b5d39b3326
spec:
  appRef:
    apiGroup: kubedb.com
    kind: Hazelcast
...
This field follows the following format: `<app group>/<resource kind>`. The above AppBinding is pointing to a `hazelcast` resource under `kubedb.com` group.
...
| `TARGET_APP_RESOURCE` | Represents the resource under that application group that this appbinding represents (i.e: `hazelcast`).                           |
| `TARGET_APP_TYPE`     | Represents the complete type of the application. It's simply `TARGET_APP_GROUP/TARGET_APP_RESOURCE` (i.e: `kubedb.com/hazelcast`). |
...
- Learn how to use KubeDB to manage various databases [here](/docs/v2026.4.13-rc.0/guides/README).
