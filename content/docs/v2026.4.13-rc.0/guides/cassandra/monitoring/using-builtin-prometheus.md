---
title: Monitor Cassandra using Builtin Prometheus Discovery
menu:
  docs_v2026.4.13-rc.0:
    identifier: cas-using-builtin-prometheus-monitoring
...
# Monitoring Cassandra with builtin Prometheus

This tutorial will show you how to monitor Cassandra cluster using builtin [Prometheus](https://github.com/prometheus/prometheus) scraper.
...
- To learn how Prometheus monitoring works with KubeDB in general, please visit [here](/docs/v2026.4.13-rc.0/guides/cassandra/monitoring/overview).
...
At first, let's deploy a Cassandra cluster with monitoring enabled. Below is the Cassandra object that we are going to create.
...
Let's create the Cassandra crd we have shown above.

```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/cassandra/monitoring/cassandra-builtin-prom.yaml
cassandra.kubedb.com/cassandra-builtin-prom created
```
...
cassandra-builtin-prom   kubedb.com/v1alpha2   5.0.3     Ready    2m45s
```

KubeDB will create a separate stats service with name `{Cassandra crd name}-stats` for monitoring purpose.

```bash
$ kubectl get svc -n demo --selector="app.kubernetes.io/instance=cassandra-builtin-prom"
NAME                                  TYPE        CLUSTER-IP      EXTERNAL-IP   PORT(S)                               AGE
cassandra-builtin-prom                ClusterIP   10.96.144.17    <none>        9042/TCP,7000/TCP,7199/TCP,7001/TCP   3m1s
cassandra-builtin-prom-rack-r0-pods   ClusterIP   None            <none>        9042/TCP,7000/TCP,7199/TCP,7001/TCP   3m1s
cassandra-builtin-prom-stats          ClusterIP   10.96.195.225   <none>        56790/TCP                             3m1s
```

Here, `cassandra-builtin-prom-stats` service has been created for monitoring purpose. Let's describe the service.

```bash
$  kubectl describe svc -n demo cassandra-builtin-prom-stats
Name:                     cassandra-builtin-prom-stats
Namespace:                demo
Labels:                   app.kubernetes.io/component=database
                          app.kubernetes.io/instance=cassandra-builtin-prom
...
Selector:                 app.kubernetes.io/instance=cassandra-builtin-prom,app.kubernetes.io/managed-by=kubedb.com,app.kubernetes.io/name=cassandras.kubedb.com
Type:                     ClusterIP
...
Endpoints:                10.244.0.29:8080,10.244.0.33:8080
...
```bash
kubectl delete -n demo cassandra/cassandra-builtin-prom
...
- Monitor your Cassandra database with KubeDB using [`out-of-the-box` Prometheus operator](/docs/v2026.4.13-rc.0/guides/cassandra/monitoring/using-prometheus-operator).
