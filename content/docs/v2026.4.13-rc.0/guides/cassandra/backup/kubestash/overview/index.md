---
title: Backup & Restore Cassandra Using KubeStash
menu:
  docs_v2026.4.13-rc.0:
    identifier: guides-cas-backup-overview-stashv2
    name: Overview
...
<figure align="center">
  <img alt="KubeDB + KubeStash" src="/docs/v2026.4.13-rc.0/guides/cassandra/backup/kubestash/overview/images/kubedb_plus_kubestash.svg">
...
<figure align="center">
  <img alt="Cassandra Backup Overview" src="/docs/v2026.4.13-rc.0/guides/cassandra/backup/kubestash/overview/images/backup_overview.png">
...
13. Then, it creates the `Job` to backup the targeted `Cassandra` database.
...
15. Then, the `Job` dumps the targeted `Cassandra` database and uploads the output to the backend. KubeStash pipes the output of dump command to uploading process. Hence, backup `Job` does not require a large volume to hold the entire dump output.

16. After the backup process is completed, the backup `Job` updates the `status.components[dump]` field of the `Snapshot` resources with backup information of the target `Cassandra` database.
...
<figure align="center">
  <img alt="Database Restore Overview" src="/docs/v2026.4.13-rc.0/guides/cassandra/backup/kubestash/overview/images/restore_overview.png">
  <figcaption align="center">Fig: Cassandra Restore Process Overview</figcaption>
</figure>

The restore process consists of the following steps:

1. At first, a user creates a `Cassandra` database where the data will be restored or the user can use the same `Cassandra` database.
...
- Backup a `Cassandra` database using KubeStash by the following guides from [here](/docs/v2026.4.13-rc.0/guides/cassandra/backup/kubestash/logical/).