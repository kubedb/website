---
title: Monitor ClickHouse using Builtin Prometheus Discovery
menu:
  docs_v2026.4.13-rc.0:
    identifier: ch-using-builtin-prometheus-monitoring
...
# Monitoring ClickHouse with builtin Prometheus

This tutorial will show you how to monitor ClickHouse cluster using builtin [Prometheus](https://github.com/prometheus/prometheus) scraper.

## Before You Begin

- At first, you need to have a Kubernetes cluster, and the kubectl command-line tool must be configured to communicate with your cluster. If you do not already have a cluster, you can create one by using [kind](https://kind.sigs.k8s.io/docs/user/quick-start/).

- Install KubeDB operator in your cluster following the steps [here](/docs/v2026.4.13-rc.0/setup/README).

- If you are not familiar with how to configure Prometheus to scrape metrics from various Kubernetes resources, please read the tutorial from [here](https://github.com/appscode/third-party-tools/tree/master/monitoring/prometheus/builtin).

- To learn how Prometheus monitoring works with KubeDB in general, please visit [here](/docs/v2026.4.13-rc.0/guides/clickhouse/monitoring/overview).
...
At first, let's deploy a ClickHouse cluster with monitoring enabled. Below is the ClickHouse object that we are going to create.
...
Let's create the ClickHouse crd we have shown above.

```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/clickhouse/monitoring/builtin-prom-clickhouse.yaml
clickhouse.kubedb.com/clickhouse-builtin-prom created
```

Now, wait for the cluster to go into `Ready` state.

```bash
➤ kubectl get ch -n demo
NAME                      TYPE                  VERSION   STATUS         AGE
clickhouse-builtin-prom   kubedb.com/v1alpha2   24.4.1    Provisioning   96s
```

KubeDB will create a separate stats service with name `{ClickHouse crd name}-stats` for monitoring purpose.

```bash
➤ kubectl get svc -n demo --selector="app.kubernetes.io/instance=clickhouse-builtin-prom"
NAME                                  TYPE        CLUSTER-IP      EXTERNAL-IP   PORT(S)             AGE
clickhouse-builtin-prom               ClusterIP   10.43.240.214   <none>        9000/TCP,8123/TCP   2m1s
clickhouse-builtin-prom-keeper        ClusterIP   10.43.30.245    <none>        9181/TCP            2m1s
clickhouse-builtin-prom-keeper-pods   ClusterIP   None            <none>        9234/TCP            2m1s
clickhouse-builtin-prom-pods          ClusterIP   None            <none>        9000/TCP,8123/TCP   2m1s
clickhouse-builtin-prom-stats         ClusterIP   10.43.105.254   <none>        9363/TCP            2m1s

```

Here, `clickhouse-builtin-prom-stats` service has been created for monitoring purpose. Let's describe the service.

```bash
➤ kubectl describe svc -n demo clickhouse-builtin-prom-stats
Name:                     clickhouse-builtin-prom-stats
Namespace:                demo
Labels:                   app.kubernetes.io/component=database
                          app.kubernetes.io/instance=clickhouse-builtin-prom
...
Selector:                 app.kubernetes.io/instance=clickhouse-builtin-prom,app.kubernetes.io/managed-by=kubedb.com,app.kubernetes.io/name=clickhouses.kubedb.com
Type:                     ClusterIP
IP Family Policy:         SingleStack
IP Families:              IPv4
IP:                       10.43.105.254
IPs:                      10.43.105.254
Port:                     metrics  9363/TCP
TargetPort:               metrics/TCP
Endpoints:                10.42.0.121:9363,10.42.0.125:9363,10.42.0.126:9363 + 4 more...
...
prometheus-547c78fc57-mg6st   1/1     Running   0          48s
```

Now, run following command on a separate terminal to forward 9090 port of `prometheus-7bd56c6865-8dlpv` pod,

```bash
$ kubectl port-forward -n monitoring prometheus-547c78fc57-mg6st 9090
Forwarding from 127.0.0.1:9090 -> 9090
Forwarding from [::1]:9090 -> 9090
```

Now, we can access the dashboard at `localhost:9090`. Open [http://localhost:9090](http://localhost:9090) in your browser. You should see the endpoint of `clickhouse-builtin-prom-stats` service as one of the targets.


Check the labels marked with red rectangle. These labels confirm that the metrics are coming from `ClickHouse` cluster `clickhouse-builtin-prom` through stats service `clickhouse-builtin-prom-stats`.
...
```bash
kubectl delete -n demo clickhouse/clickhouse-builtin-prom
...
- Monitor your ClickHouse database with KubeDB using [`out-of-the-box` Prometheus operator](/docs/v2026.4.13-rc.0/guides/clickhouse/monitoring/using-prometheus-operator).
