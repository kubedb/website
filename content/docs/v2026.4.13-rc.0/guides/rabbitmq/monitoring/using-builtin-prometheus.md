---
title: Monitor RabbitMQ using Builtin Prometheus Discovery
menu:
  docs_v2026.4.13-rc.0:
    identifier: rm-using-builtin-prometheus-monitoring
...
# Monitoring RabbitMQ with builtin Prometheus

This tutorial will show you how to monitor RabbitMQ database using builtin [Prometheus](https://github.com/prometheus/prometheus) scraper.
...
- To learn how Prometheus monitoring works with KubeDB in general, please visit [here](/docs/v2026.4.13-rc.0/guides/rabbitmq/monitoring/overview).
...
Let's create the RabbitMQ crd we have shown above.

```bash
$ kubectl apply -f https://github.com/kubedb/docs/raw/{{< param "info.version" >}}/docs/examples/rabbitmq/monitoring/builtin-prom-rm.yaml
...
```bash
$ kubectl get svc -n demo --selector="app.kubernetes.io/instance=builtin-rabbitmq"
NAME                     TYPE        CLUSTER-IP     EXTERNAL-IP   PORT(S)     AGE
builtin-rabbitmq         ClusterIP   10.99.28.40    <none>        27017/TCP   55s
builtin-rabbitmq         ClusterIP   None           <none>        27017/TCP   55s
builtin-rabbitmq         ClusterIP   10.98.202.26   <none>        56790/TCP   36s
```

Here, `builtin-prom-rm-stats` service has been created for monitoring purpose. Let's describe the service.

```bash
$ kubectl describe svc -n demo builtin-rabbitmq-stats
Name:              builtin-rabbitmq-stats
Namespace:         demo
Labels:            app.kubernetes.io/name=rabbitms.kubedb.com
                   app.kubernetes.io/instance=builtin-rabbitmq
Annotations:       monitoring.appscode.com/agent: prometheus.io/builtin
                   prometheus.io/path: /metrics
                   prometheus.io/port: 56790
                   prometheus.io/scrape: true
Selector:          app.kubernetes.io/name=rabbitmqs.kubedb.com,app.kubernetes.io/instance=builtin-rabbitmq
Type:              ClusterIP
IP:                10.98.202.26
...
Check the labels marked with red rectangle. These labels confirm that the metrics are coming from `RabbitMQ` database `builtin-prom-rm` through stats service `builtin-prom-rm-stats`.
...
- Monitor your RabbitMQ database with KubeDB using [`out-of-the-box` Prometheus operator](/docs/v2026.4.13-rc.0/guides/rabbitmq/monitoring/using-prometheus-operator).
