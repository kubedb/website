---
title: Backup & Restore Redis Using KubeStash
menu:
  docs_v2026.4.13-rc.0:
    identifier: guides-rd-backup-overview-stashv2
    name: Overview
...
<figure align="center">
  <img alt="KubeDB + KubeStash" src="/docs/v2026.4.13-rc.0/guides/redis/backup/kubestash/overview/images/kubedb_plus_kubestash.svg">
<figcaption align="center">Fig: Backup KubeDB Databases Using KubeStash</figcaption>
</figure>

## How Backup Works

The following diagram shows how KubeStash takes backup of a `Redis` database. Open the image in a new tab to see the enlarged version.

<figure align="center">
  <img alt="Redis Backup Overview" src="/docs/v2026.4.13-rc.0/guides/redis/backup/kubestash/overview/images/backup_overview.svg">
...
13. Then, it creates the `Job` to backup the targeted `Redis` database.
...
15. Then, the `Job` dumps the targeted `Redis` database and uploads the output to the backend. KubeStash pipes the output of dump command to uploading process. Hence, backup `Job` does not require a large volume to hold the entire dump output.

16. After the backup process is completed, the backup `Job` updates the `status.components[dump]` field of the `Snapshot` resources with backup information of the target `Redis` database.

## How Restore Process Works

The following diagram shows how KubeStash restores backed up data into a `Redis` database. Open the image in a new tab to see the enlarged version.

<figure align="center">
  <img alt="Database Restore Overview" src="/docs/v2026.4.13-rc.0/guides/redis/backup/kubestash/overview/images/restore_overview.svg">
...
- Backup a `Redis` database using KubeStash by the following guides from [here](/docs/v2026.4.13-rc.0/guides/redis/backup/kubestash/logical/).