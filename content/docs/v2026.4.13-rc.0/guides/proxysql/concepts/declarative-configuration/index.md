...
User will provide the configuration under the `.spec.configuration.init.inline` section of the proxysql yaml. The operator parses the yaml and creates a configuration file. A secret is then created, holding that configuration file inside. Each time a new pod is created it is created with the configuration file inside that secret.
...
You can write the configuration in yaml format under the `spec.configuration.init.inline` section and the operator would do the rest. `spec.configuration.init.inline` section is devided into four sections : `spec.configuration.init.inline.mysqlUsers` , `spec.configuration.init.inline.mysqlQueryRules`, `spec.configuration.init.inline.mysqlVariables`, `spec.configuration.init.inline.adminVariables`. You can configure the `mysql_users` and `mysql_query_rules` tables and also the global variables under the corresponding fields. This is the [api](https://pkg.go.dev/kubedb.dev/apimachinery@v0.29.1/apis/kubedb/v1alpha2#ProxySQLConfiguration) documentation. We will discuss in detail about each of the fields. 

### configuration.init.inline.mysqlUsers
...
### configuration.init.inline.mysqlQueryRules
...
### configuration.init.inline.mysqlVariables
    ...
    configuration:
      init:
        inline:
          ...
          ...
```
### configuration.init.inline.adminVariables
