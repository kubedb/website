...
`spec.version` is a required field specifying the name of the [WeaviateVersion](/docs/v2026.7.10/guides/weaviate/concepts/catalog) CR where the docker images are specified. Run `kubectl get weaviateversions` to list the versions available in your cluster.
...
`spec.authSecret` references the Secret holding the Weaviate API-key credentials. If not provided, KubeDB creates one named `<database-name>-auth` containing the standard Weaviate API-key environment variables (`AUTHENTICATION_APIKEY_ENABLED`, `AUTHENTICATION_APIKEY_ALLOWED_KEYS`, and `AUTHENTICATION_APIKEY_USERS`). You can rotate it with a [RotateAuth](/docs/v2026.7.10/guides/weaviate/rotate-auth/rotate-auth) ops request.

### spec.configuration

`spec.configuration` provides a custom Weaviate configuration (the `conf.yaml` file). It can be supplied either through a Secret (`spec.configuration.secretName`) or inline (`spec.configuration.inline`). See [Using Custom Configuration File](/docs/v2026.7.10/guides/weaviate/configuration/using-config-file).

### spec.tls

`spec.tls` enables TLS encryption using [cert-manager](https://cert-manager.io/). It references an `Issuer`/`ClusterIssuer` through `spec.tls.issuerRef`, and `spec.tls.clientAuth` controls whether mutual TLS (client certificate authentication) is required. See [Weaviate TLS](/docs/v2026.7.10/guides/weaviate/tls/overview).
...
`spec.deletionPolicy` controls what happens to the database resources when the `Weaviate` object is deleted. The available options are `DoNotTerminate`, `Halt`, `Delete`, and `WipeOut`. See the [Quickstart](/docs/v2026.7.10/guides/weaviate/quickstart/quickstart#database-deletionpolicy) for details.
...
- Learn how to use KubeDB to run a Weaviate database [here](/docs/v2026.7.10/guides/weaviate/quickstart/quickstart).
- Detail concepts of [WeaviateVersion object](/docs/v2026.7.10/guides/weaviate/concepts/catalog).
