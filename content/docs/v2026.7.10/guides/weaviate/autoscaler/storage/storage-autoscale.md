...
- Install `KubeDB` Provisioner, Ops-Manager, and Autoscaler operators in your cluster following the steps [here](/docs/v2026.7.10/setup/README).
...
- You should be familiar with the following `KubeDB` concepts:
  ...
  - [Storage Autoscaling Overview](/docs/v2026.7.10/guides/weaviate/autoscaler/storage/overview)
  - [Volume Expansion](/docs/v2026.7.10/guides/weaviate/volume-expansion/overview)
...
> **Note:** The auto-trigger relies on the `volume_used_percentage` metric being available through KubeDB's metrics API. If that metric is not exposed in your cluster, the autoscaler will stay configured and watching but will not generate an ops request. The underlying `VolumeExpansion` mechanism it uses is the same one demonstrated step-by-step in the [Volume Expansion](/docs/v2026.7.10/guides/weaviate/volume-expansion/volume-expansion) guide.
