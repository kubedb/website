...
Now, install the KubeDB operator in your cluster following the steps [here](/docs/v2026.7.10/setup/install/_index).  and make sure install with helm command including `--set global.featureGates.Hazelcast=true --set global.featureGates.ZooKeeper=true` to ensure Hazelcast and ZooKeeper crd.
...
> We have designed this tutorial to demonstrate a production setup of KubeDB managed Hazelcast. If you just want to try out KubeDB, you can bypass some safety features following the tips [here](/docs/v2026.7.10/guides/hazelcast/quickstart/overview/#tips-for-testing).
...
- `spec.deletionPolicy` specifies what KubeDB should do when a user try to delete Hazelcast CR. Deletion policy `Delete` will delete the database pods, secret and PVC when the Hazelcast CR is deleted. Checkout the [link](/docs/v2026.7.10/guides/hazelcast/concepts/hazelcast#specdeletionpolicy) for details.
...
- `AppBinding` - an [AppBinding](/docs/v2026.7.10/guides/hazelcast/concepts/appbinding) which hold to connect information for the database. It is also named after the hazelcast instance.
...
2. **Use `deletionPolicy: WipeOut`**. It is nice to be able to resume the database from the previous one. So, we preserve all your `PVCs` and auth `Secrets`. If you don't want to resume the database, you can just use `spec.deletionPolicy: WipeOut`. It will clean up every resouce that was created with the Hazelcast CR. Checkout the [link](/docs/v2026.7.10/guides/hazelcast/concepts/hazelcast#specdeletionpolicy) for details.
