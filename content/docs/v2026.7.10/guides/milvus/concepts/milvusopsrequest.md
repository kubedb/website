...
`MilvusOpsRequest` is a Kubernetes `CustomResourceDefinition` (CRD). It provides declarative configuration for administrative operations on a [Milvus](/docs/v2026.7.10/guides/milvus/concepts/milvus) database such as version updates, scaling, restarts, TLS changes, authentication rotation, and storage operations.
...
`spec.databaseRef` is required. It points to the [Milvus](/docs/v2026.7.10/guides/milvus/concepts/milvus) object the operation will run against.
...
See [Reconfigure TLS Overview](/docs/v2026.7.10/guides/milvus/reconfigure-tls/overview).
