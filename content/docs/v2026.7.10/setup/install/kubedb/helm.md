...
To keep the set as small as possible, enable only the engines you need through `global.featureGates.<Engine>` (see [Common Configuration](/docs/v2026.7.10/setup/install/kubedb/configuration)), then re-run the step 2 command (or pull the matching per-engine lists) to capture the exact image tags.
...
Next: [enable database engines and verify the installation](/docs/v2026.7.10/setup/install/kubedb/configuration).
