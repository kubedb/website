...
Now, install KubeDB cli on your workstation and KubeDB operator in your cluster following the steps [here](/docs/v2025.12.9-rc.0/setup/README) and make sure to include the flags `--set global.featureGates.Cassandra=true` to ensure **Cassandra CRD**  with helm command.
...
When you install the KubeDB operator, it registers a CRD named [CassandraVersion](/docs/v2025.12.9-rc.0/guides/cassandra/concepts/cassandraversion). The installation process comes with a set of tested CassandraVersion objects. Let's check available CassandraVersions by,
...
- `AppBinding` - an [AppBinding](/docs/v2025.12.9-rc.0/guides/cassandra/concepts/appbinding) which hold to connect information for the Cassandra. Like other resources, it is named after the Cassandra instance.
