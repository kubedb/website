...
`spec.version` is a required field specifying the name of the [MemcachedVersion](/docs/0.12.0/concepts/catalog/memcached) crd where the docker images are specified. Currently, when you install KubeDB, it creates the following `MemcachedVersion` crd,
...
- [Monitor Memcached with builtin Prometheus](/docs/0.12.0/guides/memcached/monitoring/using-builtin-prometheus)
- [Monitor Memcached with CoreOS Prometheus operator](/docs/0.12.0/guides/memcached/monitoring/using-coreos-prometheus-operator)

### spec.configSource

`spec.configSource` is an optional field that allows users to provide custom configuration for Memcached. This field accepts a [`VolumeSource`](https://github.com/kubernetes/api/blob/release-1.11/core/v1/types.go#L47). So you can use any kubernetes supported volume source such as `configMap`, `secret`, `azureDisk` etc. To learn more about how to use a custom configuration file see [here](/docs/0.12.0/guides/memcached/custom-config/using-custom-config).
...
`KubeDB` provides the flexibility of deploying Memcached server from a private Docker registry. To learn how to deploym Memcached from a private registry, please visit [here](/docs/0.12.0/guides/memcached/private-registry/using-private-registry).
